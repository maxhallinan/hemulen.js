;(function() {
  ...
  function _closest(ele, selector) {
    ...
    return (function traverseUp(el) {
      for (i = 0, j = possibles.length; i < j; i++) {
        if (possibles[i] === el) {
          return el;
        }
      }
  ...
  Hemulen.prototype._onSub = function(e) {
    ...
    if (!forms[formIndex].formSubmitted) {
      for (i = 0, j = forms[formIndex].beforeSub.length; i < j; i++) {
        forms[formIndex].beforeSub[i].callback(e, this);
      }
  ...
  function Hemulen(options) {
    ...
    if (!this.hemulenEl || this.hemulenEl.constructor !== String) {
      throw new Error('hemulenEl is a required configuration option and must be a CSS selector string.');
    }

    if (!this.namespace || this.namespace.constructor !== String) {
      throw new Error('namespace is a required configuration option and must be a CSS selector string.');
    }
  ...
  Hemulen.prototype._init = function() {
    ...
    for (a = 0, b = formEls.length; a < b; a++) {
  ...
  Hemulen.prototype._removeEventListeners = function(hemulenElId, formDataSet) {
    ...
    if (!hemulenElId || hemulenElId.constructor !== String) {
      throw new Error('This is an invalid value: ' + hemulenElId);
    }
                ...
                for (baz in storedData[foo][bar]) {
                  if (storedData[foo][bar].hasOwnProperty(baz)) {
                    ...
                    if ((typeof storedData[foo][bar][baz]).toLowerCase() === 'object' && storedData[foo][bar][baz].constructor !== Array) {
                      for (qux in storedData[foo][bar][baz]) {
  ...
  Hemulen.prototype.getFileId = function(hemulenElId, file) {
    ...
    for (key in forms[formIndex].filesStored[hemulenElId]) {
  ...
  Hemulen.prototype.deleteFile = function(hemulenElId, fileId) {
    ...
    if (!hemulenElId || hemulenElId.constructor !== String) {
      throw new Error('This is an invalid value: ', fileId);
    }
  ...
  Hemulen.prototype.addData = function(hemulenElId, fileId, updates) {
    ...
    if (!fileId || fileId.constructor !== String) {
      throw new Error('This is an invalid value: ', fileId);
    }

    if (!updates || (typeof updates === 'Object' && updates.constructor === Array)) {
      throw new Error('This is an invalid value: ', updates);
    }
